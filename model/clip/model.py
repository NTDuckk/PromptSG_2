...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, inputs):
        if isinstance(inputs, (list, tuple)):
            x = inputs[0]
            attn_out, attn_weight = self.attention(self.ln_1(x))
            x = x + attn_out
            x = x + self.mlp(self.ln_2(x))
            return [x, attn_weight]

        x = inputs
        attn_out, _ = self.attention(self.ln_1(x))
        x = x + attn_out
        x = x + self.mlp(self.ln_2(x))
        return x
...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.output_dim = output_dim
        self.input_resolution = (h_resolution, w_resolution)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, cv_emb=None):
        if isinstance(self.visual, VisionTransformer):
            return self.visual(image.type(self.dtype), cv_emb)
    ...
    def forward(self, image, text=None, cv_emb=None):
        if text is None:
            return self.encode_image(image, cv_emb=cv_emb)

        image_features = self.encode_image(image, cv_emb=cv_emb)
