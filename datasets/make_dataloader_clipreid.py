...
def make_dataloader(cfg):
    ...
    dataset_flag = getattr(cfg.DATASETS, "DATASET_FLAG", "clipreid").lower()
    if dataset_flag == "query":
        val_data = dataset.query
        num_query = len(dataset.query)
    elif dataset_flag == "gallery":
        val_data = dataset.gallery
        num_query = 0
    elif dataset_flag == "clipreid":
        val_data = dataset.query + dataset.gallery
        num_query = len(dataset.query)
    else:
        raise ValueError(
            "Unsupported cfg.DATASETS.DATASET_FLAG: {}. "
            "Expected one of ['query', 'gallery', 'clipreid']".format(dataset_flag)
        )
